...
class MesAtsDaFengEL(HPPluginBase, MesDataProcess):
    """MES对象"""
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.wsdl = self.get_config(
            "wsdl",
            "http://172.26.8.64:8888/mycim2/services/JobManagementWebService?wsdl",
        )
        self.client = None
    ...
    def connect(self):
        ...
        for ii in range(3):
            try:
                ...
                self.client = None
                self.emit(
                    HPEventType.ErrStatusChanged,
                    ErrStatus(err_type=HPErrorType.MesConnectError, is_err=True),
                )
            time.sleep(1)
    ...
    def login(self, user_name, password, facility_id):
        ...
        try:
            ...
            self.emit(
                HPEventType.ErrStatusChanged,
                ErrStatus(err_type=HPErrorType.MesConnectError, is_err=False),
            )
            if result["success"]:
    ...
    def dispatch_lot(self, el_data, vi_result=None):
        ...
        logging.info(
            "mes过账前: 组件号=%s, 图片地址=%s, 检测结果=%s", module_id, file_path, vi_result
        )
        # 图片路径映射
        ...
        trans_data_items = self._generate_trans_data_items_stand_alone(
            module_id, vi_result
        )
        ...
        try:
            logging.info(
                "dispatch_lot_for_el:{}".format(trans_data["parameter"]["items"])
            )
            logging.info("transData: %s" % trans_data)
            logging.info(
                "开始mes过账: 车间=%s, 设备=%s, 组件=%s, 数据=%s",
                self.facility_id,
                self.equip_id,
                module_id,
                trans_data,
            )
            response = self.client.service.dispatchLot(
                self.facility_id,
                self.user_name,
                self.equip_id,
                json.dumps([module_id]),
                json.dumps(trans_data),
            )
            result = json.loads(response._value_1)
            ...
            self.emit(
                HPEventType.ErrStatusChanged,
                ErrStatus(err_type=HPErrorType.MesUploadError, is_err=True),
            )
    ...
    def get_el_horad(self, el_data, vi_result):
        ...
        try:
            ...
            response = self.client.service.GetELHorad(
                json.dumps(data, ensure_ascii=False)
            )
            ...
            if vi_result.is_ng:
                ...
                if defect_selected:
                    ...
                    for defect, loc_list in defect_selected.items():
                        for loc in loc_list:
                            mes_defects.append(
                                [self.defect_label_map.get(defect), loc.split("A")[-1]]
                            )
                    ...
                    mes_defects = [
                        [self._lable_name2text(i["class"]), str(i["loc"][1])]
                        for i in vi_result.defects
                    ]
                else:
