...
class TestOpsConfig(OpsConfigBase):
    ...
    def get_station_id(self):
        ...
        :return:
        """
        station_url = "http://{host}:{port}/api/v1/stations/".format(
            host=self.host, port=self.port
        )
        ...
        station_res = requests.get(
            url=station_url, headers=self.headers, params=parm, timeout=10
        )
    ...
    def get_config_and_modify(self):
        ...
        config_url = (
            "http://{host}:{port}/api/v1/stations/{station_uid}/config/".format(
                host=self.host, port=self.port, station_uid=self.station_id, timeout=10
            )
        )
        data = {"without_config": "true"}
    ...
    def modify_config(self, key, value):
        ...
        config_url = (
            "http://{host}:{port}/api/v1/stations/{station_uid}/config/".format(
                host=self.host, port=self.port, station_uid=self.station_id
            )
        )
        requests.put(
            url=config_url,
