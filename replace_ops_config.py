...
class OpsConfigBase:
    ...
    def login(self):
        ...
        :return:
        """
        login_url = "http://{host}:{port}/api/v1/users/tokens/".format(
...
class ModifOpsConfig(OpsConfigBase):
    ...
    def get_config_and_modify(self):
        ...
        for station in self.stations_id:
            ...
            config_url = (
                "http://{host}:{port}/api/v1/stations/{station_uid}/config/".format(
                    host=self.host, port=self.port, station_uid=station_id, timeout=10
                )
            )
            data = {"without_config": "false"}
            station_config_res = requests.get(
                url=config_url, data=data, headers=self.headers
            )
            station_config_res = station_config_res.json()
            station_config_str = (
                station_config_res["data"]["config"]
                .encode("utf-8")
                .decode("unicode_escape")
            )
            # print('station_config_str',station_config_str)
            self.cfg_yaml = yaml.load(station_config_str, Loader=yaml.FullLoader)
            save_path = "./config__jiangsu/{}_{}_{}.yaml".format(
                station["facility_id"], station["product_line"], station["equipment_id"]
            )
    ...
    def put_config(self, station_id):
        for i in os.listdir("D:/Script/put_config"):
            ...
            config_url = (
                "http://{host}:{port}/api/v1/stations/{station_uid}/config/".format(
                    host=self.host, port=self.port, station_uid=station_id
                )
            )
            requests.put(
                url=config_url,
                data=dict(config=yaml.dump(self.cfg_yaml), config_desc="修改ops配置"),
...
def modif_template_to_customize(old_config_dir):
    ...
    customize_config_dict = {
        "camera_station.defect_recipe": "VIPostprocess.DefectRecipePlugin.defect_recipe",
    ...
    template_config = ConfigYaml(
        "C:Users/LiuWanpin/Desktop/plugin_config/config__隆基江苏串检.yml"
    )
    for i in old_config_file_list:
        ...
        for (
            customize_old_config,
            customize_new_config,
        ) in customize_config_dict.items():
            customize_config_value = config.get_config(customize_old_config, "")

            # 在site_local中的配置项
            if "pre_process" in customize_old_config:
                site_config = ConfigYaml(
                    "./site_local/{}.yml".format(i.split(".")[0][8:11])
                )
                customize_config_value = site_config.get_config(
                    customize_old_config, ""
                )
